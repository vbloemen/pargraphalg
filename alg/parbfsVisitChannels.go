...
// Data type for ParBFSVC.
type ParBFSVC struct {
...
func NewParBFSVC(procs int) *ParBFSVC {
	C := make([]int64, 1e8)
	N := make([]int64, 1e8)
	V := make([]int64, 1e8)
	return &ParBFSVC{C: C, N: N, V: V, Ci: 0, Cn: 0, Ni: 0, Nn: 0, procs: procs,
		mu: &sync.Mutex{}}
}

// returns whether state was already in the set,
// and adds it to the set otherwise.
func (b *ParBFSVC) findOrPut(state int64) bool {
	if atomic.CompareAndSwapInt64(&b.V[state], 0, 1) {
		return true
	}
	return false
}
...
func (b *ParBFSVC) proc(g graph.Graph, doneC chan bool, rangeC chan int64) {
...
// start again.
func (b *ParBFSVC) Run(g graph.Graph, from int) {
	// init search setup
	b.C[0] = int64(from)
	b.V[from] = 1
